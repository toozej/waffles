...
REPO_URL := https://github.com/toozej/waffles

# Detect the OS and architecture
OS := $(shell uname -s)
ARCH := $(shell uname -m)
LATEST_RELEASE_URL := $(REPO_URL)/releases/latest/download/waffles_$(OS)_$(ARCH).tar.gz
ifeq ($(OS), Linux)
	OPENER=xdg-open
else
	OPENER=open
...
vet: ## Run `go vet` in Docker
	docker build --target vet -f $(CURDIR)/Dockerfile -t toozej/waffles:latest . 

test: ## Run `go test` in Docker
	docker build --progress=plain --target test -f $(CURDIR)/Dockerfile -t toozej/waffles:latest . 

build: ## Build Docker image, including running tests
	docker build -f $(CURDIR)/Dockerfile -t toozej/waffles:latest .

get-cosign-pub-key: ## Get waffles Cosign public key from GitHub
	test -f $(CURDIR)/waffles.pub || curl --silent https://raw.githubusercontent.com/toozej/waffles/main/waffles.pub -O

verify: get-cosign-pub-key ## Verify Docker image with Cosign
	cosign verify --key $(CURDIR)/waffles.pub toozej/waffles:latest

run: ## Run built Docker image
	docker run --rm --name waffles --env-file $(CURDIR)/.env toozej/waffles:latest
...
distroless-build: ## Build Docker image using distroless as final base
	docker build -f $(CURDIR)/Dockerfile.distroless -t toozej/waffles:distroless . 

distroless-run: ## Run built Docker image using distroless as final base
	docker run --rm --name waffles -v $(CURDIR)/config:/config toozej/waffles:distroless

install: ## Install waffles from latest GitHub release
	if command -v go; then \
			go install github.com/toozej/waffles@latest ; \
	else \
			echo "Downloading waffles binary for $(OS)-$(ARCH)..."; \
			mkdir -p $(CURDIR)/tmp; \
			curl --silent -L -o $(CURDIR)/tmp/waffles.tgz $(LATEST_RELEASE_URL); \
			tar -xzf $(CURDIR)/tmp/waffles.tgz -C $(CURDIR)/tmp/; \
			chmod +x $(CURDIR)/tmp/waffles; \
			sudo mv $(CURDIR)/tmp/waffles /usr/local/bin/waffles; \
			rm -rf $(CURDIR)/tmp; \
	fi
...
local-release: local-test docker-login ## Release assets using locally installed golang toolchain and goreleaser
	if test -e $(CURDIR)/waffles.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && goreleaser release --rm-dist; \
	else \
		echo "no cosign private key found at $(CURDIR)/waffles.key. Cannot release."; \
	fi

local-sign: local-test ## Sign locally installed golang toolchain and cosign
	if test -e $(CURDIR)/waffles.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && cosign sign-blob --key=$(CURDIR)/waffles.key --output-signature=$(CURDIR)/waffles.sig $(CURDIR)/out/waffles; \
	else \
		echo "no cosign private key found at $(CURDIR)/waffles.key. Cannot release."; \
	fi
...
local-verify: get-cosign-pub-key ## Verify locally compiled binary
	...
	cosign verify-blob --key $(CURDIR)/waffles.pub --signature $(CURDIR)/waffles.sig $(CURDIR)/out/waffles

local-install: local-build local-verify ## Install compiled binary to local machine
	sudo cp $(CURDIR)/out/waffles /usr/local/bin/waffles
	sudo chmod 0755 /usr/local/bin/waffles

upload-secrets-to-gh: ## Upload secrets from .env file to GitHub Actions Secrets + Dependabot
	$(CURDIR)/scripts/upload_secrets_to_github.sh waffles 

upload-secrets-envfile-to-1pass: ## Upload secrets and .env file to 1Password
	$(CURDIR)/scripts/upload_secrets_to_1password secrets waffles
	$(CURDIR)/scripts/upload_secrets_to_1password envfile waffles
...
pre-commit-run: ## Run pre-commit hooks against all files
	...
	go-licenses report github.com/toozej/waffles/cmd/waffles
	govulncheck ./...
